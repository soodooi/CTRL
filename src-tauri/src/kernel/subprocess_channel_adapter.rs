...
//
// Inverse direction (Op inbound from a transport → SubprocessActor) belongs to
// the capability owner, which translates `agent_prompt` / `env_signal` into
// `SubprocessStdin` / `SubprocessSignal` mailbox events. This adapter owns only
// outbound Channel translation. (ADR-001 spine §4 v22)
...
/// Lifecycle status projected by a SubprocessActor channel forwarder.
/// The adapter updates it when a `SubprocessExit` event is observed.
...
///
/// The forwarder also updates `status` when it sees `SubprocessExit`, keeping
/// Channel lifecycle projection synchronized without a second process owner.
/// (ADR-001 spine §4 v22)
pub async fn forward_subprocess_outbox(
    ...
    // If the loop exits with status still Running, the forwarder ended without
    // a terminal event. Promote the generic channel projection to Crashed so a
    // future consumer cannot observe stale lifecycle state.
...
/// status. Exit code 0 is treated as Stopped; non-zero or missing is Crashed.
/// (ADR-001 spine §4 v22)
async fn update_status_from_exit(
    ...
    let new_status = match (exit_code, spawn_error) {
        // exit 0 with no spawn_error = clean termination.
...
fn translate_op(op: Op, bridge: &EventWsBridge, stream_id: &str) {
    match op.kind {
        ...
        OpKind::SubprocessSpawned => {
            ...
            let payload = match op.payload {
                serde_json::Value::Object(mut map) => {
                    map.insert("state".into(), serde_json::Value::String("running".into()));
        ...
        // Inbound-only kinds (transport → actor); the actor never emits these
        // outbound. Both internal Subprocess* and wire-facing families share
        // the same invalid-outbox contract, so drop them defensively.
...
mod tests {
    ...
    async fn exit_code_zero_marks_stopped() {
        ...
        assert_eq!(
            *status.lock().await,
            EnvLifeStatus::Stopped { exit_code: Some(0) }
        );
    }
