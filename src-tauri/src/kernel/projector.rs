...
pub fn project_into_dir(dir: &Path, port: &str, token: &str) -> std::io::Result<bool> {
    project_gate_into_dir(dir, port, token, resolve_byo_intent().as_deref())
}

/// Like `project_into_dir` but with an EXPLICIT intent scope. The base workspace
/// projection passes the global default; the per-pack projection (§1B.8) passes
/// the pack's OWN domain, so a driver launched in a pack's directory sees exactly
/// that pack's capability subset — no widening of the global default.
fn project_gate_into_dir(
    dir: &Path,
    port: &str,
    token: &str,
    intent: Option<&str>,
    ...
    let next = kernel_entry(port, token, BYO_CLI_CALLER, intent);
...
pub fn project_agents_md(dir: &Path) -> std::io::Result<bool> {
    project_agents_block(dir, &ctrl_agents_block())
}

/// Upsert a specific CTRL-managed block into `<dir>/AGENTS.md` (marker-delimited,
/// preserving user prose). The base workspace passes `ctrl_agents_block()`; the
/// per-pack projection (§1B.8) passes `pack_agents_block(...)` so the pack's
/// directory carries the pack's own context.
fn project_agents_block(dir: &Path, block: &str) -> std::io::Result<bool> {
...
/// The AGENTS.md context block for a PER-PACK scope (§1B.8) — names the pack, its
/// dedicated KB (if any), and its capability domain, so a driver launched in the
/// pack's directory knows what it is working in and that its view is scoped.
fn pack_agents_block(name: &str, kb: Option<&str>, intent: &str) -> String {
    ...
    format!(
        "{AGENTS_BEGIN}\n\
# CTRL feature pack — {name}\n\
\n\
This directory is scoped to the **{name}** feature pack (a CTRL product-grade\n\
unit). The `ctrl-kernel` gate mounted here (see `.mcp.json`) is scoped to this\n\
pack's capability domain (`{intent}`) — you see this pack's tools, not the whole\n\
workbench. Calls are permission-checked + audited at the kernel gate.\n\
{kb_line}\
- **Local is truth.** Data is plain text; read local first, writes are visible\n\
  immediately.\n\
\n\
This block is generated by CTRL and refreshed on launch. Add your own notes\n\
OUTSIDE the markers — that content is preserved.\n\
{AGENTS_END}"
    )
}
...
mod tests {
    ...
    fn per_pack_gate_carries_the_packs_own_intent_not_the_global_default() {
        // A pack scope projects the pack's OWN domain (e.g. `source`), which is
        // NOT in the global BYO default — proving per-pack scope grants exactly
        // the pack, without widening the base workspace.
        let dir = TempDir::new().unwrap();
        let wrote =
    ...
    #[test]
    fn pack_agents_block_names_the_pack_its_kb_and_scope() {
        let block = pack_agents_block("Ghostfolio", Some("Finance/Ghostfolio"), "source");
        assert!(block.contains("# CTRL feature pack — Ghostfolio"));
        assert!(block.contains("Finance/Ghostfolio/"));
        assert!(block.contains("`source`"));
        // Marker-delimited like the base block (so it upserts, never duplicates).
        assert!(block.contains(AGENTS_BEGIN) && block.contains(AGENTS_END));
    }

    #[test]
    fn pack_agents_block_omits_kb_line_when_absent() {
        let block = pack_agents_block("CF Workers", None, "cli");
        assert!(!block.contains("Knowledge base:"));
        assert!(block.contains("# CTRL feature pack — CF Workers"));
    }

    #[test]
    fn per_pack_agents_md_writes_block_and_preserves_user_prose() {
        let dir = TempDir::new().unwrap();
        fs::write(dir.path().join("AGENTS.md"), "# My own notes\nkeep me\n").unwrap();
        let block = pack_agents_block("Ghostfolio", Some("Finance/Ghostfolio"), "source");
        let wrote = project_agents_block(dir.path(), &block).unwrap();
        assert!(wrote);
        let out = fs::read_to_string(dir.path().join("AGENTS.md")).unwrap();
        assert!(out.contains("keep me"), "user prose must survive");
        assert!(out.contains("# CTRL feature pack — Ghostfolio"));
    }
