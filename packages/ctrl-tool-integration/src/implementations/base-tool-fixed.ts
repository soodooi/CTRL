...
export class HttpTool extends BaseTool {
  ...
  protected async onStart(): Promise<void> {
    ...
    this.axiosInstance = axios.default.create({
      timeout: (this._manifest as any).extensions?.http?.timeout_ms || 10000,
      headers: (this._manifest as any).extensions?.http?.headers || {}
    });
  }
  ...
  protected async onRun(options: ToolRunOptions): Promise<any> {
    if (!(this._manifest as any).extensions?.http) {
      throw new Error('HTTP tool configuration not found');
    }
    
    const { endpoint, method } = (this._manifest as any).extensions.http;
...
export class ToolFactory implements IToolFactory {
  async createTool(manifest: ToolManifest): Promise<ITool> {
    switch (manifest.type) {
      ...
      case 'cli':
        // 简化版本，不实现完整的CLI工具
        throw new Error('CLI tool implementation not yet available');
      case 'mcp':
