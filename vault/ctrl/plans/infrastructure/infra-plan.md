...
type: infra-plan
status: planning (non-authoritative; decisions live in ADR-002)
owner: bao
related: [ADR-002 §provider, ADR-002 §composition §7.4, ADR-002 §crypto(mesh)]
viz: docs/design/ctrl-infra-plan.html
---

# CTRL 云侧基建规划

> 结论先行:**功能包管理 / 反馈 / 分享 绝大部分留 Cloudflare(已有栈,边缘,R2 出口免费,scale-to-zero);AWS 只需 1 台小机做「中国可达出口代理」——CF Workers 地理够不到 CN 金融站的那个洞。** 可视化 = `docs/design/ctrl-infra-plan.html`。
