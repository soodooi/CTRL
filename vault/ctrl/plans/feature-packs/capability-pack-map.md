...
purpose: 汇总功能包历史规划与候选优先级，作为非权威计划输入；当前决定以 ADR-002 §7 和 owning ADR 为准。
...
> **非权威计划。** 本文只汇总历史规划、候选范围与优先级，不定义架构铁律、固定模块清单或当前开发目标。当前目标读 `vault/ctrl/GOAL.md`；功能包架构读 ADR-002 §7；通讯边界读 ADR-010。
>
> 读法:本文回答「曾规划哪些功能包」。GOAL.md = 当前活跃目标；owning module ADR = 唯一架构权威；本文 = 功能包这一维的计划输入。
