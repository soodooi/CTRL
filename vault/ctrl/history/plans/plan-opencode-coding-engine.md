...
author: claude
status: retired
purpose: 历史 OpenCode/BYO driver 方案记录；不作为当前 coding engine 规范。
related:
...
historical_governing_claims:
  - ADR-005 §8.7 (former Irisy engine proposal)
  - ADR-001 spine §byo-cli-driver (former BYO-CLI driver proposal)
---

> **Status: Retired historical record.** Non-authoritative; this is not the current coding-engine decision or implementation plan. Current ownership lives in ADR-001, ADR-002, ADR-003, and ADR-005 as applicable.

> The BYO-driver and ACP-engine alternatives below are retained only as provenance. Their stale version references and checklists do not govern current work.

# 历史提案:coding 引擎 = OpenCode
