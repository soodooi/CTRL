> **Status: Retired historical record.** Non-authoritative; this is not the smart-table governing truth. Current smart-table and frontend decisions live in ADR-002 and ADR-003.

# 智能表格 — 历史对标规划图

> bao 2026-06-21:「你不要一个一个,我看不清楚,整体架构想清楚没有?对标哪个产品?做到一模一样还是有哪些差距?现在进度如何?」
> 本文件曾作为智能表格对标的整体规划图，现仅保留历史设计依据，不再指导实施。
