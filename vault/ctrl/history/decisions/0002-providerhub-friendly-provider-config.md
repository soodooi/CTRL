...
> **历史决策记录，已退出 active authority（2026-08-02）。** 下文的 `Accepted` 和 `Decision` 仅描述当时状态；当前 provider 决定以 ADR-002 §3 为准，配置 UI 以 ADR-003 为准。
...
- 待:provider 写进 `~/.hermes/.env`(已做适配器),但 hermes 认不认 volc/zhipu 自定义 base_url 需真机验证 → [open-questions](../inventories/open-questions.md)
