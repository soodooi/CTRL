...
kind: design
status: retired
superseded_by: "vault/ctrl/adrs/010-communication.md (ADR-010 communication v11)"
note: "Historical protocol diagnosis only. Current communication authority is ADR-010 v11; current substrate implementation boundaries are in ADR-002."
...
> **Status: Retired historical record.** Non-authoritative; this diagnosis does not describe the current protocol surface or migration state. Current communication authority is ADR-010 communication v11; current substrate boundaries live in ADR-002.

# 全项目通讯协议重构 — 历史分析

> 这是 2026-06-22 的代码盘点快照，数字与组件名称仅作 provenance，不用于当前设计或验收。

<details>
<summary>Archived four-verb/ST-SS proposal — superseded by ADR-010 v11</summary>
...
- 采用 **MCP 资源语义**(resources/read=query · resources/subscribe=subscribe · tools/call=produce · list=describe)+ **CQRS 读写分离**(query/command/event)做背书 —— 不自造协议(见 the contemporaneous pipeline study 调研)。
...
3. **逐 source 收敛**(= the contemporaneous pipeline study 的能力管线,一条一 PR + 测试):
