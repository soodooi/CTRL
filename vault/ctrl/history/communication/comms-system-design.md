---
title: CTRL 通讯系统设计 + 迁移图（historical execution reference）
kind: design
created_at: 2026-06-24
owner: bao
author: claude
status: retired
purpose: 历史执行图；当前通信决策与 endpoint spec 的唯一权威是 ADR-010 communication v11，substrate 实现边界由 ADR-002 决定。
current_authorities:
  - vault/ctrl/adrs/010-communication.md
  - vault/ctrl/adrs/002-substrate.md
---

> **Status: Retired historical record.** Non-authoritative; this execution diagram does not define communication truth. Current decisions live in ADR-010, with substrate implementation boundaries in ADR-002.

# CTRL 通讯系统设计 + 迁移图（历史执行参考）

<details>
<summary>Archived 2026 execution snapshot — not current architecture or acceptance</summary>

> 读法:这是当时的**执行图**(目标终态 + 全量分类 + 迁移序列)。当前决策只读 ADR-010；当前实现边界只读 ADR-002。
...
```
   ...
   语义契约(SSOT,自创) │  §14 = describe / query / produce            │  实现一次/源
        ...
        ┌────────────────────────────────┼────────────────────────────────┐
   wire/物化(分缝): MCP(工具)       typed schemas + bindings(本机)   protobuf(跨设备)
        │ JSON-RPC+JSON Schema       Rust command/event schemas       over WebRTC + E2EE
        │ tools/list = 端点 spec     Tauri IPC/Channels/WS registry  (缝⑧,独立目标)
        │ (缝④⑤及⑦工具面)           (缝①③⑥)
        ▼
   ╔═════════════════════════════════════════════════════════════════════════╗
...
**自创只两块**（无标准覆盖，正当）：§14 语义 SSOT + :17873 gate。其余复用实际 transport 与既有类型系统：MCP/JSON Schema、Tauri IPC/Channels/WS、protobuf。**红线：不自造 wire/IDL，不维护第二份 schema/binding truth。**
...
| **流**(Irisy 回复/终端/进度) | `query{watch}` 投影 | **typed external-event schema + Tauri Channels/WS binding** | 授权回 gate | Channels/WS 订阅 |
...
### C. 流 → typed external events + binding registry（缝③⑥，SC6）
- `stss::*`(4:subscribe/publish/list_streams/get_bridge_token)→ **退役**，改 WS/Channels + generated event/binding artifacts。
...
|---|---|---|---|---|
| **A 地基** 🔨 | gate 治理(SC1/2/3)+ MCP schema artifact + 棘轮 lint + ADR-010 v10 | 散 → 收口 | MCP artifact 已落；command/event/binding artifacts 待 D | — |
...
| **D 流与端点物化**(缝③⑥,SC6) | ST-SS 活线 → WS/Channels；`query{watch}` 落地；生成 command/event schemas + `stream-bindings` | ST-SS → typed transport bindings | 前端流(终端/Irisy)视觉不回归 + artifacts 与 adapter registry drift check 通过 | ST-SS 退役 |
...
- 🔨 **A 地基部分完成**：两信任域类型 + 审计 ledger + 可见性裁剪 + projector stamp + MCP schema artifact + 棘轮已落；ADR-010 已到 v10；command/event/binding artifacts 待 D。
- 🔨 **B 退面**：`gate_invoke` 桥与大部分退面已在后续实现完成；以当前代码/GOAL 为准，本历史图数字不可作现状 inventory。
- 🔨 **C §14 盖全**：以 ADR-002 §14 与当前 GOAL 为准，本历史图的 2026-06-24 数字已过时。
- ⚠️ **D 流与端点 artifacts**：ST-SS 已 de-brand；typed bus 生产接线及 generated command/event/binding artifacts 未完成。
- ⬜ **E 跨设备**：独立目标，未起。
...
- **端点 spec = 运行真相的生成物**：MCP `tools/list` + §14 `describe` + Rust typed command/external-event schemas + adapter `stream-bindings`；catalog 不爬源码。
