> **Status: Retired historical record.** Non-authoritative; this document is provenance only. Current communication authority is ADR-010 communication v11; current substrate implementation boundaries are in ADR-002.

# CTRL 通讯架构 — 历史内核设计

> 本文中的设计已收编或被取代，不再作为 live truth、implementation spec 或 governing decision。
>
> ~~Status: ACCEPTED v2 (2026-06-22)~~
> Current owners: ADR-010 communication v11 and the relevant current ADR-002 section.
> 历史摘要: **一个窄腰、两个信任域、三个动词、N 个传输、降级即契约。**
...
## 6. 六个已拍板的工程决策 (全部守 `.kiro/steering/development-philosophy.md` 保命线)
