...
| `produce` | 2 | probable write | Mutate or act on one canonical CTRL ResourceRef. Call describe first; operation must match an advertised schema. A call from an autonomous caller passes through ReviewGate; direct user-surface actions are governed by the existing gate policy instead. |  |
