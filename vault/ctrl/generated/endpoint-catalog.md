...
- **100** MCP tools registered on the `:17873` gate
- **17** tools recognized by this generator's curated §14 face list
- **34** probable writes / **66** probable reads, classified by endpoint-name heuristic
- **170** registered Tauri commands
- **2** exact-name overlaps between MCP tools and Tauri commands
...
## Tauri command registration by module (170 total)

This table is generated from `src-tauri/src/commands/mod.rs`. Exact-name overlap with an MCP tool is an inventory signal only; architectural interpretation belongs to the owning ADR.
...
| `commands/coding_launcher.rs` | 2 |
| `commands/gate.rs` | 1 |
| `commands/pack_registry.rs` | 1 |
| `commands/chat.rs` | 1 |
| `commands/chat_attachment.rs` | 1 |
| `commands/coding_attachment_picker.rs` | 1 |
| `commands/image.rs` | 1 |
