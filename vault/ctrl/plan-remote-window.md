...
## ToDesk/RustDesk 对标 → 持久设备模型(2026-07-07,bao「是todesk最佳实践吗」质疑 → 调研)

调研 RustDesk(开源权威)+ ToDesk + TeamViewer 的**连接/寻址/配对**模型(非像素层):

- **传输我做对了(保持)**:relay-only **不做 P2P/打洞**对纯 JSON 是对的 —— P2P 是它们为省视频带宽才做,带来 NAT 检测 + 对称/CGNAT 失败(RustDesk 最被吐槽 #6689),手机上照样退回 relay;桌面出站 0 监听 = RustDesk 的注册姿势;E2E 密钥在链接 fragment = 等价但更简。**不加 P2P = 避免过度工程**。
- **配对我做错了一半(已修)**:临时「Start session 一次性链接」= **有人值守(attended)**模型;但主用例「随时用手机连自己桌面」= **无人值守(unattended)** —— 一次性链接要在桌面上点才生成,可你远程时人不在桌面,自相矛盾。RustDesk/ToDesk/TeamViewer **全部默认持久设备 ID + 固定密码 + 后台常在线**。
- **实装(feat/remote-window 续)**:`remote-identity`(桌面持久 deviceId + E2E 密钥 + 6 位 passcode,localStorage,**待迁 Keychain**)· 「Stay reachable」保活出站 relay 连接(退避重连,room=deviceId)· 稳定链接手机 bookmark · passcode 在 **E2E 之后**由桌面校验(relay 看不到)· Reset passcode = 软撤销记住的手机 · 一次性「Share once」保留作 attended/分享 · 手机记住 passcode 静默重连。TOTP 二次验证 v1 不做(过度)。
