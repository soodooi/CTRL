...
CTRL 的 brain 拆成**两条并存的路**：① Irisy 路 = CTRL bundle + lazy-install 的
Hermes Agent；② BYO-CLI 路 = 用户自带 CLI（Claude Code 等）经 projection 接入。
CTRL 是让这两条 brain 都看得见你全部资产、且回流可被 gate 的**投影层** —— 它本身
**不再 supervise、不再编排** agent loop（loop 由两条 brain 各自自带）。
...
### 1. driver = 用户自选本地 CLI（BYO-CLI 路）
- driver 是用户**本机已安装**的强 CLI agent。**Claude Code 是旗舰对接目标**。
- 在 BYO-CLI 路径上，CTRL **不 lazy-install** driver、**不 supervise** driver 的
  agent loop（driver 自己带 loop）。driver 的升级、计费、模型选择全在用户侧 ——
  CTRL 搭便车（ride upgrades），不背维护成本。
- 注意：这跟 Irisy 路（Hermes 是 bundle brain，§2）不冲突 —— 两条路并存。
...
| 技能 (skills) | 物化 `SKILL.md` **(未落地)** | CLI 的 skills 目录 |
| 记忆 (memory) | 派生 **`AGENTS.md`** 注入 context (刻意不写 `CLAUDE.md`, driver-agnostic) | CLI 启动时读取的 context 文件 |
| 工作流 (用户主动触发) | 物化 slash command **(未落地)** | `.claude/commands/` |
...
| 技能 | `SKILL.md` 文件 | CLI skills 目录 | 可 override | 🔲 设计目标, 未落地 (现走 gate 工具 `skill_list`/`skill_read`/`skill_write`) |
| 记忆 | 派生 `AGENTS.md` (刻意不写 `CLAUDE.md`, driver-agnostic) | CLI context 文件 | 可 override | ✅ 已落地 `projector.rs` (+ Codex `~/.codex/config.toml`) |
| 工作流（主动触发） | slash command | `.claude/commands/` | 可 override | 🔲 设计目标, 未落地 |
