...
剥离历史仍可从 Git 查阅；不要直接重新激活旧 runtime。先由 bao 建立新目标/ADR，明确 Kiro-compatible 的协作需求，再按最小机制实现。当前多会话开发只采用 `vault/ctrl/development/team-workflow.md` 的 worktree + PR 模式。
