...
**N4 实录(2026-07-11,代码侧完成;活体验收窗=下两个交易日)**:
- **close.py 复盘升级到五模块模板**(与 daily-review skill 对齐):①大盘环境概览(广度/涨跌停/涨停池/活跃度/赚钱效应两指标)②板块主线(涨停梯队+龙头的板块聚类,散点日如实显示)③连板梯队(**2 板起算连板,首板 N 家另计**——词汇纪律落进代码)④情绪周期(阶段中文名/温度/炸板率/判读)⑤次日策略(**规则引擎按周期阶段给姿态,无 LLM 可运行**,ChanStock 参照的规则兜底思路;合规词汇)+ 强制风险提示尾注。
- **新增结构化复盘行**:每次运行往 `tables/stocks-review.md` 追一行(date/mood_stage/zt_count/max_streak/attribution 全真值),**按日期幂等**(重跑替换不重复)。daily note 复盘段同样幂等。
- **验证**:隔离 test-vault 干跑两遍——五模块渲染正确、重跑后 section=1/行=1、计数取整;AI-signal 死端口走 skip 路径正常。修一处真 bug:`first_boards` 是计数不是列表(`len()` 崩 → `int()`)。
- **活体验收窗**:plist 只跑工作日(周一至五 18:30),今天周六不触发——**连续两天全绿 = 周一(07-13)+ 周二(07-14)18:30**,含 AI-signal 步(代理修复后首次真跑)。
- **数据观察(backlog)**:leaders 出现「托伦斯 858.85%」异常涨幅(疑新股/除权口径),leaders 工具的 pct 口径待核,非本片范围。

**N5 实录(2026-07-11,已完成——持仓手记表填活,「我持仓怎么样」真机)**:
- **执行环境**:PR #173 已合并,headless kernel(:17999)直接用 main 检出构建(带 N2-0 修复 + boot 重连钩子,118 工具上 gate);全程 caller=irisy(用户面,review-gate 豁免),审计 26 行在册(add_field / append_row / update_cell / stock_quote / query)。
- **表 schema 补列(经 gate `smart_table_add_field`)**:positions 加 `last`(Last, number)/ `updated`(Updated, text)/ `account`(Account, text——paper 标记模拟账本,§5「模拟账本」落地);重跑幂等(already exists 拒绝)。小坑:add_field 参数名是 `type` 不是 `cell_type`;pack 工具经 gate 返回**双层包裹**(下游 MCP 结果整体作 text 再包一层),客户端要解两层。
- **模拟账本种子(全真数)**:watchlist + positions 各 2 行(300065 海兰信 / 600519 贵州茅台),cost=prev_close、现价=07-11 收盘真报价(海兰信 28.79 **+20.01%**——N3 会诊「判决日=二板确认」剧本当日真兑现;茅台 1204.98 +1.93%)。盈亏/配比经 gate `update_cell` 逐格填活:pnl 20.01% / 1.93%,weight 19.28% / 80.72%,`smart_table_query` 回读 2 行全对。
- **🐛 顺手修一个真数据 bug**:8 表系统 reference 列的目标表写的是**裸文件名**(`table: stocks-watchlist.md`),而 `augment_relations` 按 vault 根解析 → 目标表读不到 → lookup(positions.name 等)恒空。修 = 4 处 schema 改 vault 相对路径(`tables/stocks-…`),改后 lookup 真解析(name=海兰信/贵州茅台)。注:lookup 是查询时注入的纯派生、不回写 md,文件里 Name 列为空是设计使然。内核侧「裸名相对源表目录解析」留 backlog(数据修好后非必需)。
- **收盘自动刷新(close.py 2c 段)**:解析 positions 表 → `main.stock_quote` 批量取现价 → 重算 Last / PnL % / Weight % / Updated 逐格覆写(天然幂等),异常永不打断复盘主流程;`CTRL_STOCK_VAULT` env 可换 vault 根做隔离干跑(service 仍从 canonical vault 加载)。隔离 test-vault + 死端口 gate 干跑:**positions: 2/2 rows repriced**,真 vault 未被污染。
- **intro.md** 增补「我持仓怎么样」入口 + 持仓手记节(管理非交易、Account=paper、收盘自动 + 随时问两条刷新路径),镜像已同步 `~/.ctrl/mcps/ctrl-stock-cn/`。
- **验收对照**:「持仓手记表 + 盈亏列填活」✅「我持仓怎么样」真机 ✅(headless=真 kernel,同 N3 honest note:桌面 app 重启后即同款路径);连续自动刷新活体窗 = 周一 07-13 18:30 收盘任务(与 N4 复盘同窗)。
