...
### backlog（记着,非本片）
- **gate `skill_read` 并发挂起 bug**：`skill_read` 经 :17873 gate 间歇性挂起(实测 2:10 / 20s SSE 心跳),而底层 `read_local_skill` 读 9KB 文件本身是毫秒级、`vault_read` 同一时刻秒回 → 根因在 gate 层串行化 / head-of-line blocking,不是 skill_read 实现或文件大小。**影响全局/个人 skill 层**(pack skill 已绕开走 vault_read 不受此阻),需单独一片修 gate 并发。避免再被 Irisy 「读取超时」的自造解释带偏(账本才是真相,memory `feedback-read-audit-ledger-not-guess-irisy`)。
