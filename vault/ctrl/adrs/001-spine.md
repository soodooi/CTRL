...
changelog:
  ...
  - v18 2026-07-27: **§4 amendment — Coding's drag-drop attachments ride the ACP protocol layer's new capability-negotiated ContentBlocks (bao "拖拽要的" + "这个输入协议层，Irisy和opencode是都要用的"), governing decision at ADR-002 substrate §1.8.6 v75.** A file dropped into the Coding chat composer is read client-side and passed to `coding_chat_stream` as an attachment alongside the turn's text; `coding_chat.rs` forwards it into `AcpClient::prompt()`'s new attachment parameter, which resolves to an `Image` or `EmbeddedResource` ContentBlock only if the connected `opencode acp` session negotiated that capability, matching the SAME mechanism Irisy's own engine now has available (ADR-002 §1.8.6, ADR-005 §8.7 v31) — no Coding-specific protocol code, purely a consumer of the shared `AcpClient` capability. No manifest schema change, no new gate tool. Pairs ADR-003 frontend §8.5 v35 (the drag-drop UI) and ADR-005 irisy §8.7 v31.
  - v17 2026-07-27: **§4 amendment — opencode gets a Coding-specific capability brief so it actually knows CTRL's gate exists (bao: "先要给 opencode 一些文档,规范是 CTRL 的规范,前端是 CTRL 的工作区前端" — audited what opencode could see over the v16 ACP connection and found it was zero).** v16 wired `session/new`'s `mcpServers` to pass CTRL's `:17873` gate to opencode (the same passthrough hermes/codex/claude-code get) and made opencode a legitimate ACP-driven engine — but `coding_chat.rs`'s `client.prompt(&turns, None, ..)` passed `None` for the system preamble, so opencode had the WIRE to the gate with zero briefing on what it was for: no mention of `gate_tool_search`/`gate_tool_call`, the governed `create-feature-pack` SKILL.md playbook, the manifest form contract, or the shared `FeaturePackScene` frontend rendering contract. It behaved as a generic coding assistant that happened to have some extra MCP tools it didn't know the purpose of. Fix: a new `CODING_CAPABILITY_BRIEF` constant in `coding_chat.rs` (deliberately separate from Irisy's own `CTRL_CAPABILITY_BRIEF` in `acp_client.rs` — different task, different content: Irisy operates the user's data, opencode writes CTRL feature-pack manifests/code) points opencode at: (1) `gate_tool_search`/`gate_tool_call` as the full ~100-tool discovery path, (2) `skill_list`/`skill_read` the `create-feature-pack` skill as the REQUIRED authoring authority before freehanding a pack, (3) a brief summary of the three manifest forms (`actions[]`/`record_source`/`mcp-server`) and the `config_schema` secret-field contract (never in the manifest body), (4) the `FeaturePackScene` rendering contract — `record_source`/`workspace.table_prefix`/`knowledge_base` are declarative fields that choose the frontend rendering, not something a pack author writes bespoke UI code for. Passed via `client.prompt(&turns, Some(CODING_CAPABILITY_BRIEF), ..)`, injected only on the session's first turn (the existing `AcpClient.primed` flag, unchanged). Verified: `cargo test --lib` 507/507 (2 new: `brief_points_at_the_governed_authorities`, `brief_never_tells_opencode_to_put_secrets_in_the_manifest`); a real end-to-end probe (spawn the installed `opencode acp` binary, feed the brief text through the identical prompt shape `coding_chat.rs` sends, ask "what should you read first before authoring a manifest?") returned "The `create-feature-pack` skill." — confirms the brief actually lands in a live opencode session, not just that the string compiles into the binary. No manifest schema change, no new gate tool, no frontend change — purely closes the "opencode has the wire but not the map" gap. Pairs ADR-002 substrate § 7.4 v34 (the skill this brief points at) and ADR-005 irisy §9 v25 (the sibling brief this mirrors the pattern of, without duplicating its content).
  - v16 2026-07-27: **§4 amendment — Coding drives opencode over ACP instead of an embedded PTY, superseding v15 within the same session (bao: verify against real products before committing; a real probe found opencode speaks ACP natively, and CTRL already runs the identical protocol for Irisy's engine).** v15's embedded-xterm PTY reproduced actual, reproducible rendering failure on the real machine (a black terminal area even though the underlying `opencode` process and its MCP handshake were confirmed alive in the kernel log) — the SAME class of failure the v13/v27 external-terminal move had been trying to duck in the first place, now hit a second time. Root-caused instead of patched again: web-searched and directly verified (not from documentation alone) that third-party OpenCode desktop clients (e.g. `opencode-nexus`, a Tauri + Astro/Svelte app) universally integrate via `opencode serve`'s HTTP+SSE API or ACP, never by rendering the CLI's own TUI inside a host app — and a direct local probe (`opencode acp`, three real JSON-RPC round trips: `initialize` -> `session/new` -> `session/prompt`) confirmed the installed binary's `acp` subcommand speaks genuine Agent Client Protocol, streaming `agent_thought_chunk` / `agent_message_chunk` in the exact shape `shell/acp_client.rs` already parses for Irisy's engine (hermes/codex/claude-code, ADR-005 §8.7). Fix: `CodingScene` no longer embeds a PTY at all — it drives `opencode acp` through a NEW, independent `AcpClient` singleton (`coding_singleton()`, separate from Irisy's `singleton()` so switching one engine never evicts the other) rooted at the selected workspace's cwd (not the vault root), rendering the same structured events (text/thought/tool-call/tool-result) as native React, reusing the ToolStepView/thinking-trace pattern AmbientHome already has for Irisy. This eliminates PTY/xterm/TERM-environment rendering risk entirely — there is no terminal emulation layer left in the Coding module. Workspace switching still confirms before resetting (killing the live ACP session, ADR-003 §8.5 v31's UX, carried forward unchanged). The v27/v31 external-terminal launch and launcher-hide behavior remain as the secondary "Open externally" action. Verified end-to-end against the real installed binary (`opencode` 1.18.5): a live `cargo test opencode_acp_smoke -- --ignored` drives `AcpClient::start_in("opencode", ..)` (the exact path `coding_chat.rs` uses) and receives a real streamed reply with `stopReason=end_turn`. Pairs ADR-002 §1B.8 v74 (unchanged), ADR-003 §8.5 v32, ADR-005 §8.7 v30.
  - v15 2026-07-27: **§4 amendment — Coding returns to an embedded PTY as its primary surface, superseding v13's external-terminal-first launcher (bao: OpenCode belongs left of the always-resident Irisy column, matching every other module's [work area | Irisy] shape; not a new pattern — this is v18's original shape, restored with the v27 stability fixes applied instead of skipped).** v13/v27 moved Coding to an external-terminal launcher specifically to duck real PTY stability bugs (dead PTY on tab-flip, un-refit xterm grid, StrictMode double-spawn, stale cwd on workspace switch) — but that traded away product cohesion: the launched OpenCode window floated outside CTRL, overlapping the always-on-top launcher panel (ADR-003 §1.1) with no shared frame. bao confirmed the goal was never "launch externally" but "Coding is the same kind of module as Notes/Tables" — content on the left, Irisy always on the right. Fix: `CodingScene` now embeds `CodingTerminal` running the user's `opencode` directly as the primary surface (still no supervision — CTRL only spawns the PTY and pipes it), with the ACTUAL stability fixes landed this time: the PTY effect keys on command+args+cwd BY VALUE (not array reference, so a parent re-render can't respawn it), workspace switching triggers a confirm step before killing a live session (no silent kill), and the terminal fills its own primary work area (validated at the size Notes/Tables' outbar actually renders, not a small Quick-Terminal disclosure). Opening a separate OS terminal/editor remains available as a secondary, collapsed "Open externally" action — v27's launch mechanism is not deleted, only demoted. Pairs ADR-002 §1B.8 v74 (unchanged — the projection mechanism this consumes), ADR-003 §8.5 v31, ADR-005 §8.7 v29.
  - v14 2026-07-26: **§4 amendment — installed feature-pack scopes become valid OpenCode launch targets, correcting v13's "not advertised as OpenCode workspaces" (bao "A"; independent review confirmed the prior state was a dead end, not a design choice).** v13 made the configured root the sole OpenCode scope because only it carried `opencode.json`; a review of the shipped launcher confirmed the diagnosed consequence — every generated pack scope (ADR-002 §1B.8) was listed as a workspace with its primary action permanently disabled, contradicting §7.5's "feature pack = product-grade project unit". Fix: `projector::project_pack` (ADR-002 §1B.8 v74) now also writes `opencode.json` into each pack's scope, stamped with the pack's own intent domain; `coding_launcher_status`/`launch_coding_workspace` discover and validate any direct child of the configured root carrying `opencode.json` as an additional launch target, using the SAME canonical-containment + opencode.json predicate for both discovery and validation (closing the discovery/validation symlink mismatch an independent review also flagged) so a workspace it advertises is always one it accepts. The configured root remains a launch target unconditionally (it does not require `opencode.json` to be discoverable, only to be OpenCode-launchable); nothing else that lacks `opencode.json` (e.g. `Notes/`, `tables/`) is ever a valid target. Launch still requires an explicit user action per workspace and still does not install, wrap, monitor, restart, or supervise OpenCode. Pairs ADR-002 §1B.8 v74, ADR-003 §8.5 v28, ADR-005 §8.7 v28.
  - v13 2026-07-25: **§4 + invariant #11 — Coding launches the user-owned OpenCode process in an external terminal first (bao approved option A), superseding v10/v18's PTY-first presentation without changing projection ownership.** The configured CTRL root is the sole OpenCode launch scope because it owns `opencode.json`; feature-pack directories retain their accepted generic `.mcp.json` + `AGENTS.md` projection and are not advertised as OpenCode workspaces. Launch requires an explicit user action, uses a fixed app allowlist and canonical root validation, and does not install, wrap, monitor, restart, or supervise OpenCode. The embedded PTY remains only an explicit Quick Terminal fallback for short shell tasks. Pairs ADR-003 §8.5 v27 and ADR-005 §8.7 v27.
...
related:
  ...
  - vault/ctrl/history/brainstorm/system-self-evolution-2026-06-04.md
---

> **Spine — immutable**. New session reads §1. All module-specific detail in module ADR (see `vault/ctrl/adrs/INDEX.md`).
...
L1 CTRL Kernel — Rust microkernel (thin: project + gate, NOT supervise the driver)
                 ...
                 + event_ws.rs (CBOR-over-WebSocket for browser/mobile) + Tauri Channels (desktop)
                 + notes_index (SQLite FTS5, optional — native Notes owns primary)
...
- **Coding launch** (v16) — the primary surface drives the user's own `opencode` over Agent Client Protocol (its native `acp` subcommand), through a dedicated `AcpClient` singleton independent of Irisy's own engine session. No PTY, no terminal emulation: CTRL renders the engine's structured events (answer text, reasoning, tool calls/results) as native React in the Coding scene's work area, left of the always-resident Irisy column. CTRL spawns the process and relays its ACP events; it does not wrap, drive, or supervise the agent loop. A workspace is selectable when it is the configured root OR a direct child of it carrying `opencode.json` — every installed feature-pack scope qualifies (ADR-002 §1B.8 v74 projects `opencode.json` alongside `.mcp.json` into each pack's directory), and is now also the engine's cwd. Switching workspaces while a session is live requires an explicit confirm (it ends and restarts the ACP session in the new cwd). Opening a separate OS terminal/editor is a secondary, collapsed action, not the primary path.
...
Projection is **intent-scoped** (project the relevant subset, don't blast full context — v1). "apps" (Feishu / Notion / OAuth / OPC connectors) are MCP **sources** (§3), not a 4th face. Notes is a face consumer, not the driver itself. **ACP is not deleted — demoted to a future "ACP-aware CLI enhancement channel"**: an ACP-speaking driver may get richer live wiring, but the default + baseline is config projection (above), which works with any BYO CLI and satisfies the vim-test.
...
11. **Brain = 2 parallel paths (v8 纠正; RETRACTS v7's blanket "no CTRL-bundled brain").** (a) **Irisy path** — CTRL **does** bundle + lazy-install + launch **Hermes Agent** as Irisy's brain (dashboard `:17890`); hermes is NOT retired. (b) **BYO-CLI path** — the driver is the user's **own** local CLI (Claude Code 等), which CTRL does NOT install or supervise; CTRL only **projects** assets into its native config (tools → `.mcp.json` pointing at gate `:17873`; skills → driver skills dir; memory → `CLAUDE.md`/`AGENTS.md`; workflows → slash command). Both paths gate at `:17873`. Projection targets are user-visible plain text (git-diffable, vim-editable). Pi remains retired (no `~/.ctrl/pi/`, v4). The v4 aggregator's kernel-SUPERVISED 3-agent model stays retired (CTRL supervises no brain); kairo stays retired. **OpenCode returns in v10 as the CODING engine — but ONLY as a BYO-CLI DRIVER** (the user's own `opencode` in the projected configured root, gate projected into `opencode.json`, no supervision), not the v4 supervised agent. **v16 drives that path over Agent Client Protocol via `opencode acp` (a dedicated AcpClient singleton, independent of Irisy's own engine session; any installed feature-pack scope with `opencode.json` per ADR-002 §1B.8 v74 qualifies as a workspace too), rendered as native React with no PTY/terminal emulation, with a secondary external-terminal action retained from v13/v27.** hermes-for-Irisy is the only BUNDLED brain. Note that §4.1's "ACP demoted to future enhancement channel" describes Irisy's right-region engine selection only — opencode's ACP integration here is the Coding module's PRIMARY mechanism, not a future channel.
...
| 5 | **System self-healing** | kernel/runtime error → selected current engine diagnostic → typed remediation + verify + rollback (ReCiSt 4-layer) |
...
6. Learn        Consolidate to markdown playbook + SQLite audit ledger + current-engine context
```
...
- **Run (full auto, deferred)**: Loops 5 (kernel/runtime recovery through typed current-engine and gate actions) + 6 (cross-user telemetry, opt-in) ship after audit ledger has weeks of evidence and Memory-R1-style RL fine-tune signal.
