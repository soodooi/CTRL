...
- Tauri command `provider_set_active(role, provider_id)` runs `trial_verify()` using the production 1-token `"hi"` path before committing. Provider setup plus first output share one absolute 30-second deadline; failure keeps the previous binding and surfaces the specific timeout/auth/network/model error. Configured presence, credential readiness, and active binding are separate states. (v68)
