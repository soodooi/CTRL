...
|---|---|---|---|---|---|
| [001](./001-spine.md) | spine | 4-layer kernel + 5 primitives + 4 mcp sources (legacy ST-SS retired v9) + **2 parallel brain paths (Irisy=Hermes bundled + BYO-CLI driver projection) + OpenCode as a BYO-CLI coding engine driven over ACP (no PTY/terminal emulation), workspace targets = configured root + installed feature-pack scopes carrying opencode.json, external launch retained as secondary, a Coding-specific capability brief pointing it at gate_tool_search/create-feature-pack/manifest forms/FeaturePackScene, and Coding's feature-pack requests executing the governed research → confirm → validate → install → smoke lifecycle without speculative form menus (§4 v19), plus drag-drop attachments consuming the shared ACP capability-negotiated ContentBlock layer** + native Notes + **3-capability-face (MCP/API/Skills via :17873 bus)** + **versioned JSON Schema manifest contract (§5 v12)** + 6 self-evolution loops | v19 | accepted | 2026-07-28 |
| [002](./002-substrate.md) | substrate | **Hermes 是 Irisy 的脑 (§1, v28 纠正 — hermes 不退役) + BYO-CLI driver projection 附加 (§1B, §1B.8 v74 per-pack scope 同时投影 opencode.json) + ACP future (§1.8) + §1.8.6 v75 capability-negotiated multi-modal ContentBlock attachments (Image/EmbeddedResource), shared by Irisy and opencode through one AcpClient** · capability surface · 3-capability-face · provider router · crypto · subprocess · MCP bus (= projection gate) · composition · **§7 v73 versioned draft-2020-12 JSON Schema is the sole cross-language manifest authority; Ajv/Rust validators are consumers** · **§12 diagnostics projection (v72: existing ACP/PTY/vault owners publish content-free lifecycle metadata, no ownership transfer)** · **§14 Unified Operation Interface (describe/query/produce; §14.13 v45 统一写侧 RecordSink + 类型化 ProduceOp — smart-table/task/calendar/docs 四源一套动词)** + **§brain v38 — Irisy 引擎泛化为可选 ACP engine (Hermes 默认 bundled / Codex / Claude Code)** + **§1.9 v46 Notes 全原生替代 — Obsidian connector 退役 (曾 16 工具, 已删), Tolaria-parity 功能面 + LRA 参考端点 E1-E13, governing `notes-module-replacement-plan.md`** + **§3 v61 claude-oauth 订阅 provider 移除 (Anthropic ToS — 订阅 OAuth 不得作 LLM provider; Anthropic 仅 BYOK API key) + §3.10 v67 provider/model 完整目录由 Models.dev 动态刷新（API-key 可表示子集），bundled 21 项仅作离线 floor；Z.AI/Coding Plan 保持独立 + §3 v69 provider 激活使用单一 30 秒真实首输出预算，并对明确支持的 OpenAI-compatible provider 仅在 trial 禁用 reasoning，正常聊天保持模型推理行为 + §3 v71 verification evidence 与 role intent 在同一 active-providers v4 中独立持久化，legacy binding fail-closed 为 unverified；仅 explicit+verified candidates 进入统一 router，所有生产 text chat 禁止直连 adapter；ProviderHub 正交显示 configuration/runtime/verification/roles** + **§7 v76 feature-pack authoring research required to land in `Research/feature-packs/<slug>.md` (create-feature-pack SKILL.md v1.2.0), not a manifest field** | v76 | accepted | 2026-07-27 |
| [003](./003-frontend.md) | frontend | Single PWA + **macOS fixed Accessory app + hidden-before-first-order full-screen launcher NSPanel (§1.1 v29: `LSUIElement`, `visible:false/focus:false`, convert/configure while hidden, `show_and_make_key()` sole macOS presentation path, no runtime policy switching)** + **Ambient morphing home (§8 SHIPPED) + ACP-driven Coding chat (no PTY) with a multi-workspace selector, one native macOS multi-file-and-folder picker (files = ACP attachments; folders = OpenCode path references, §8.5 v37), and secondary external-launch disclosure; Irisy's chat surface gains a session tab bar + bottom-toolbar agent selector (§8.6 v36)** + Sidebar L1; Notes = inline viewer + open-in-Obsidian (v9); **§6 smart-table → intelligent table (multi-view + AI field shortcuts + soft links, benchmarked vs Feishu Bitable, v10); §6.5 Irisy operation surface (`smart_table.*` gate tools, validated-string params + schema-resource semantic layer; AI column = async job triple + 100-row cost gate + merge-by-row write-back, benchmarked vs Dify/Coze/ChatBI/Airtable + MCP SEP-1686, v13); §6.5 reframed as first impl of ADR-002 §14 Unified Operation Interface (v14); §14 first vertical SHIPPED — kernel query engine + smart-table describe/query/produce gate tools, describe=tool not resource (v15); §14 FULL build shipped + reviewed PASS — 4 RecordSources (smart-table/KB/registry/providers) on one shared engine + AI-column async job, as-built reconcile of the §6.5.4 locks (v16); §6.5.4 closes bounded-concurrency + AuthFailed-stop (v17) + merge-by-snapshot row identity (v18); §6.2 view-state read/write loop closed + frontend views persistence (v19)**; **v21 Notes = 薄 KB 层(废 GraphView/CommandPalette 克隆,留树/搜索/反链 + auto-save + 文件夹管理 + 折叠树;修「每个笔记开成智能表格」)+ vault 根用户可配置(原生选择器指向自己的 Obsidian vault,默认仅 fallback)+ 同步=组合(git 助手 + Auto-sync;mesh 留 v1.1+ 仅 CTRL 自有态),事实源 `notes-module-plan.md`**; **§9 diagnostics surface (v26): typed Tauri status/smoke/trace/capture/export-preview and a metadata-only developer harness; no raw-log UI** | v37 | accepted | 2026-07-28 |
| [004](./004-cap.md) | cap | Mcp execution model + Tauri authenticated updater download + **CTRL-owned macOS atomic in-place transaction with durable health-gated rollback and compile-time-isolated signed localhost A/B fault harness (§2 v11)** + 4-layer × 3-tier auto-update (**Layer 2 = external-agent tier, ACP probe + L3, §3 v2**) + **§1 OS sandbox on pack shell DRIFT closed (macOS sandbox-exec, v3)** + **§2 clean-source provenance, split secret/citation governance baselines, retry-safe updater pairing, fail-closed updater trust epochs, and governed macOS release-identity epochs with encrypted recovery** | v11 | accepted | 2026-07-25 |
| [005](./005-irisy.md) | irisy | **PWA persona shell** + sycophancy filter + system-prompt injection + drill-down; **v11 数字员工 operator** (§8 terminal-essence dialog + §9 mission + knowledge system) + **§8.7 v31 left-region Coding drives opencode over ACP (its own AcpClient singleton, no PTY) over a selectable workspace (configured root + installed feature-pack scopes), external launch as a secondary action; right-region Irisy engine unchanged; the shared AcpClient's new multi-modal attachment capability (ADR-002 §1.8.6 v75) is now wired into Irisy's own composer + a new multi-session tab bar (Kiro-parity redesign, §8.7 v32)** + **§2 mobile = describe-driven SDUI (generic) + irisy chat over tunnel (v22); §2 remote co-view SHIPPED — semantic "remote window" (option B), relay-only + E2E + persistent device model (RustDesk/ToDesk parity) + hosted PWA app.ctrlapplab.com; pixel remote-desktop = ADR-010 ⑧, unbuilt** + **v24 engine authority = agent_installer/acp_client + source-pinned release probe; v25 planning map is non-authoritative, module ADRs remain sole architecture authority** + **v26 §8.6.1a live diagnostics projection: ACP lifecycle metadata only, observed without starting or blocking the engine** | v32 | accepted | 2026-07-27 |
...
| [010](./010-communication.md) | communication | **统一窄腰 (§14 契约 + :17873 治理 + MCP 插件协议) over 多元传输** — 8 条缝传输选型；subscribe = query{watch}；内外协议哲学分离；coding 为第⑦外部缝 (ACP)。**v5 ST-SS 弃用**→Tauri Channels+WS，protobuf 仅跨设备腿。**v7 typed trust boundary**：`InternalMsg` + EventBus 与 GateRequest 隔离，caller/intent 默认收紧。**v10 endpoint spec 收敛**：AsyncAPI 退役；工具=MCP `tools/list` JSON Schema，§14=`describe` schema，本机 command/event=Rust typed schemas + Tauri IPC/Channels/WS binding registry，跨设备=protobuf；catalog 从生成 artifacts 构建、不爬源码、不留第二份 schema/binding truth；**v11 diagnostics**：一个 Rust-owned composer 观察 Irisy/Coding/Notes；typed Tauri 全控制面，`:17873` 仅 status/smoke/trace；递归脱敏、限时内存留存、无默认磁盘/云遥测。通讯总纲，实现真相引用 001/002/003 | v11 | accepted | 2026-07-25 |
