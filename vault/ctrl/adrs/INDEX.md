...
| [005](./005-irisy.md) | irisy | **PWA persona shell** + sycophancy filter + system-prompt injection + drill-down; **v11 数字员工 operator** (§8 terminal-essence dialog + §9 mission + knowledge system) + **§2 mobile = describe-driven SDUI (generic) + irisy chat over tunnel (v22); §2 remote co-view SHIPPED — semantic "remote window" (option B), relay-only + E2E + persistent device model (RustDesk/ToDesk parity) + hosted PWA app.ctrlapplab.com; pixel remote-desktop = ADR-010 ⑧, unbuilt** + **v24 engine authority = agent_installer/acp_client + source-pinned release probe; v25 planning map is non-authoritative, module ADRs remain sole architecture authority** + **v26 §8.6.1a live diagnostics projection: ACP lifecycle metadata only, observed without starting or blocking the engine** | v26 | accepted | 2026-07-25 |
...
| frontend | PWA shell, L1 nav, Keyboard, vault browser, viewers, macOS Regular launcher/NSPanel/tray lifecycle | `packages/ctrl-web/` + `src-tauri/src/shell/{lifecycle,tray,window}.rs` + `src-tauri/src/lib.rs` + `scripts/debug/diagnostics_smoke.py` |
