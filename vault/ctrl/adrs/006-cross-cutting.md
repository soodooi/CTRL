...
changelog:
  ...
  - v8 2026-06-25: **§1 acceptance — clear the volc-paid-fallback code remnant (ADR↔impl drift fix).** v3 (2026-06-09) retired the "irisy.fallback = CTRL-paid volc" path and banned any CTRL-paid auto-fallback, but the kernel still hardcoded `CTRL_FALLBACK_PROVIDER_ID = "volc"` (registry.rs) — and `volc` had also been dropped from BUILTIN_MANIFESTS, so the default fallback pointed at a manifest that is never seeded (the out-of-box path was simply broken: a fresh user's fallback resolved to nothing). bao 2026-06-25 surfaced it ("not every user has a Volc key — you're designing a system"). Fix: the fallback slot now seeds to local `ollama` (0 CTRL cost, 0 vendor key, device-first per former CLAUDE.md derived rule #2 — runs offline), which IS the v3 posture: no CTRL-paid fallback; the only always-available zero-cost substrate is the user's own machine (CTRL bootstrap ships hermes3:8b). A CF Workers AI / CTRL-brand cloud provider may take the slot later, but only behind the ctrl-cloud secrets pipeline. Also unified all credential reads through `registry::read_credential` (encrypted vault, not the OS keyring — the keyring apple-native path silently returned nothing under the signed CTRL.app, so three call sites never saw a stored key). No new direction — completes v3. (Note: `.claude/rules/mcp-llm-reference.md` Pattern D still names CF Workers AI as the default — superseded by v3/v8, left as historical reference.)
...
Write-ops to connected products still gate (intent → review → approve → execute; §4 autonomy ladder). The CRM/ERP examples are connectors an OPC plugs in, not products CTRL ships. **(Correction 2026-07-02, bao「我们是飞书而非集成飞书」: Feishu is NOT such a connector — CTRL natively REPLACES Feishu's core and IS the local/private/AI-native Feishu: Smart-table = Bitable, Notes = docs. Connect targets = the OPC's own products CTRL does NOT replace, e.g. CRM/ERP/Ghostfolio. Feishu stays in this doc only as the COMPETITOR/incumbent contrast (lines above); IM-reach to contacts on 微信/飞书 is a separate legit axis, §2 Integration-priority. Memory `feedback-ctrl-is-feishu-not-integrate-feishu`.)**
...
- **Dependency hygiene.** Third-party deps keep their own licenses (AGPL is compatible with MIT/Apache/BSD deps). An AGPL dep like Teable stays reference-only, never vendored (ADR-002 v30). Calling a user's self-hosted AGPL service (e.g. Ghostfolio) over REST is NOT derivative work — no AGPL contagion to CTRL.
- **§5.1.1 Scoped AGPL-vendoring exception — the Tolaria notes frontend (v11, bao 2026-07-02「前端就用 tolaria」)**: the "reference-only, never vendored" rule gets ONE scoped exception — the Tolaria frontend (`refactoringhq/tolaria`, AGPL-3.0-or-later) is vendored as `packages/ctrl-notes-ui` to serve as CTRL's notes-module UI (its Rust backend + CLI-integration layer are NOT taken; CTRL's kernel is the only backend via an adapter over its ~49-command surface). Rationale: AGPL→AGPL is fully license-compatible; the frontend's maturity (editor armor: IME/paste/render-recovery, types/views, git UI, whiteboard) is the expensive-to-rebuild half, while CTRL's backend (gate/§14/audit/FTS5) is already superior. **Accepted cost**: `ctrl-notes-ui` carries third-party copyright → that package is PERMANENTLY AGPL (no future dual-licensing of it); the rest of the core keeps full relicensing freedom. **Containment rules**: vendored code stays inside `packages/ctrl-notes-ui` (never spreads into kernel / other packages / MIT feature packs); upstream LICENSE + copyright notices retained + `UPSTREAM.md` records the forked commit; Tolaria TRADEMARKS (name/logo) stripped before any user-visible ship (AGPL grants no trademark rights); AGPL §5 modification notices on changed files. Upstream posture: snapshot fork + per-release cherry-pick (no continuous-sync promise — upstream is alpha with ~10 releases/day; our patches concentrate in the adapter layer to keep cherry-picks feasible).

**Supersedes** the former CLAUDE.md hard rules "License: All Rights Reserved / all sub-packages private:true + UNLICENSED / no `npm publish` of `@ctrl/*`". The former CLAUDE.md was updated at the time to match.
...
- [x] Mcp priority list lives in this ADR + memory; the standalone spec was deferred under the former CLAUDE.md flexible-development contract. Closed as deferred.

### Plain-text (§3)
- [x] former CLAUDE.md "Design Philosophy" section references this ADR + spells out derived rules. Closed.
