...
| Control family | Mockup surface | Intent | Handler | Authority / endpoint | Status | Evidence or required correction |
|---|---|---|---|---|---|---|
| Find/Installed/Create tabs, live search filtering, categories, Close, `···` menu | capabilities | U14, U16, U18 | `Discover` local state | Local presentation state over the mounted catalog result | `local` | Tabs and filters need UI evidence, not backend endpoints. Menu items require their own rows once defined. |
...
| `Find FCTs` search submit | capabilities | U14 | No search-submit handler confirmed | No owner operation confirmed | `declared` | Production search filters the mounted catalog locally. Remove this submit button or implement a distinct query and its unavailable state. |
