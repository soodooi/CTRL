...
| 2026-05-25 | **Amend (fourth校准, release/0.1.39): kernel lean — over-design audit + Design Philosophy codification.** Web-researched 2026 industry signals confirm: (a) WASM cannot uniformly sandbox mixed-language subprocesses (Node/Python/Rust/Bash) — [Shayon 2026 sandbox isolation guide](https://www.shayon.dev/post/2026/52/lets-discuss-sandbox-isolation/), [Wasmer comparison](https://wasmruntime.com/en/compare/wasmtime-vs-wasmer); Raycast / VSCode / Obsidian all use process isolation. (b) Tauri 2 already provides Capability + Isolation Pattern + CSP — layering WASM = double-sandbox no benefit ([Tauri Security docs](https://v2.tauri.app/security/)). (c) MCP confirmed as 2026 industry-standard tool protocol — McpHost + McpServer (ADR-013) NOT over-designed. (d) When v1.x truly needs WASM, WasmEdge (8MB, 1.5ms cold start) is the right runtime, not wasmtime (80MB+). **Removed:** `kernel/sandbox.rs` + wasmtime + cranelift deps (0 usage in v1); `kernel/composition.rs` (0 usage, violates derived rule #4 "One-shot, not flows" — composition belongs in brain agent loop or user-authored hermes-skill, not kernel); Ollama dropped from default LLM fallback chain (BYOK still works on demand). **Design Philosophy locked (this section is canonical for v1):** (1) **Subprocess + Tauri ACL > WASM** for plugin isolation — re-use Tauri Capability + Isolation Pattern + CSP, don't double-sandbox. (2) **Kernel atomic, composition in brain/skill** — kernel commands are one-shot per derived rule #4; multi-step workflow lives in Pi agent loop or user-authored hermes SKILL.md, not kernel. (3) **MCP is the tool wire** — both inbound (kernel as MCP server, ADR-013) and outbound (kernel as MCP host, ADR-010). (4) **Lean kernel** — only ship substrates v1 actually uses; defer unused with explicit comment + research-cited rationale (no speculative "we'll need it later"). Estimated kernel binary -8MB, cold cargo build -2.5 min. Spine + 5-primitive set + 10 invariants unchanged. |
