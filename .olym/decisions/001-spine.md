...
CLI driver — user's own resident CLI (Claude Code 等) · auto-discovers projected assets on launch
brain — none bundled (v7: hermes / opencode / Pi retired) · Obsidian = notes/KB editor (user's own, v24)
```
...
> **Evolution**: Pi-centric (retired pre-v4) → 3-agent aggregator hermes/opencode/kairo (v4–v6, **RETRACTED**) → **BYO-CLI driver platform (v7)**. CTRL is a **projection** layer, not a brain host.
...
- **CLI driver** ★ — the user's **own** resident strong CLI (Claude Code 等). CTRL ships / lazy-installs / supervises **no** brain (v7 — hermes / opencode / Pi retired). Two triggers share one projection: **passive** (user runs `claude` themselves → CLI auto-discovers projected assets, zero-intrusion, satisfies vim-test) + **active** (Ctrl-summoned ephemeral workspace launches the CLI). Scheduling/decision权 stays in the CLI model.
...
11. **No CTRL-bundled brain / agent install dir** (v7 — RETRACTS v4's `~/.ctrl/agents/<name>/` 3-agent lazy-install of hermes / opencode / kairo). The driver is the user's **own** local CLI (Claude Code 等) — CTRL does not install or supervise it. CTRL instead **projects** assets into the driver's native config (tools → `.mcp.json` pointing at gate `:17873`; skills → driver skills dir; memory → `CLAUDE.md`/`AGENTS.md`; workflows → slash command). Projection targets are user-visible plain text (git-diffable, vim-editable). ACP demoted to future enhancement channel (§4.1).
