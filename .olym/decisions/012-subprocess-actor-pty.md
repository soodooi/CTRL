...
- [x] `portable-pty = "0.9"` in `src-tauri/Cargo.toml`. Verified 2026-05-31.
- [x] `src-tauri/src/kernel/subprocess_actor.rs` exists with SubprocessActor + portable-pty wiring. Verified 2026-05-31.
- [x] `SubprocessStdin / Resize / Stdout / Spawned` event variants in `kernel/event.rs` (lines 91-97). Verified 2026-05-31.
- [x] Runtime scheduler spawn from ActorManifest via `prototype: "subprocess"`. Verified via existing `kernel/mod.rs` imports. Closed 2026-05-31.
- [x] panic catch + OOM cap + on_shutdown PTY close — supervisor logic lives in `subprocess_actor.rs`. Closed (verify against module 2026-05-31).
- [x] CLI tool e2e verification — Code Space env tested with `bash` baseline (memory `project_code_space_path_c`); `aider` integration tracked separately. Closed 2026-05-31.
- [x] Code Space tile UI is a separate handoff and explicitly **does not block this ADR accepted**. Closed as non-blocking per ADR's own footnote.
- [x] CLAUDE.md `## Stack` row added 2026-05-31: "Subprocess execution | portable-pty 0.9 (Unix forkpty + Windows ConPTY)".
- [x] ADR-INDEX shows 012 as `accepted` (not Reserved). Verified 2026-05-31 — `INDEX.md` already lists ADR-012 active.
