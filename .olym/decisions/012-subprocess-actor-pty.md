...
`ActorManifest.prototype = "subprocess"` 时，`initial_state` 含 `command` (可执行文件名/路径)、`args` (启动参数, 支持 env 占位)、`env` (附加环境变量, 含 `CTRL_TILE_ID`)、`cwd` (工作目录, 支持 `$HOME` 占位)、`pty` (`cols`/`rows` 终端尺寸)。

*(Manifest example elided — see `.olym/specs/tool-manifest/spec.md` for schema; runtime parse in `src-tauri/src/kernel/subprocess_actor.rs`.)*
