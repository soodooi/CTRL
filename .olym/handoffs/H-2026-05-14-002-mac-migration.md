...
- Win 代码参考 (在 `feat/h-001-e-cleanup` 分支, 跟你 base 的 main 不同): use `git show origin/feat/h-001-e-cleanup:<path>` to read the Win-shipped versions of `src-tauri/src/lib.rs` (Win `run()`), `shell/hotkey.rs` (WH_KEYBOARD_LL impl — your `mac_impl` mirrors this shape), `shell/lifecycle.rs` (boot order + hotkey install), `commands/` (entire commands tree), `Cargo.toml` (tauri-plugin-global-shortcut etc.), `capabilities/` (default + keychain JSON), and `tauri.conf.json` (main window config). Or check the branch out read-only via `git switch -d origin/feat/h-001-e-cleanup` (detached HEAD) and `git switch feat/h-001-mac-migration` to return.
...
Win 那边 `shell/hotkey.rs` 用 WH_KEYBOARD_LL hook + state machine (`ctrl_pending` / `other_seen` / `ctrl_down_at`). Mac 用 CGEventTap 完成同样事情 — 在 `shell/hotkey.rs` 加 `#[cfg(target_os = "macos")] mod mac_impl`, 引入 `core_graphics::event::CGEventTap` + `core_foundation::runloop::CFRunLoop`, 暴露 `MacTap::install(callback)` — 内部状态机 (`TAP_THRESHOLD_MS = 400`) 跟 Win impl 严格一致: 创建 `cgSessionEventTap` (keyDown | keyUp | flagsChanged), 用专属线程跑 CFRunLoopRun, 在 callback 里检测 Control flagsChanged + 运行 state machine + fire `OnTap` on lone-Ctrl detection.

*(Pseudocode block elided. Implementation: `src-tauri/src/shell/hotkey.rs` `mac_impl` (shipped in commit `e68950b`).)*
...
观察日志 `/tmp/ctrl-mac-smoke.log` 节选: ShellLifecycle boot 起 kernel daemon → KernelSupervisor ready (kernel + WS bridge :17872) → StssBridge serve failed (port-in-use, 预期, 两份实例并存) → tray icon installed → lone-Ctrl hotkey 安装 + CGEventTap enabled + CFRunLoop 运行 → 9 次 lone-Ctrl tap, 9 次 WindowController toggle SHOW/HIDE, 状态机 0 误触.

*(Log snippet elided — full trace lives at `/tmp/ctrl-mac-smoke.log` during the 2026-05-19 smoke run.)*
