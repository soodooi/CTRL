...
**Required change**: enum 加 `custom` 选项 (10th value), manifest 声明 `custom_component_path: string` 字段 (required when `ui='custom'`). 创作者贡献 keycap 可写自己的 tab component, 符合 thesis "创作者扩展" 哲学.

*(Zod schema sketch elided — landed in spec v0.2 §4.3.)*
...
**Required change**: §3.3 默认路径改为 `pip install hermes-agent` (PyPI v0.14.0 zeus 2026-05-22 已 verify). 5 步 bootstrap_hermes 流程: check `python3 ≥ 3.11` → 创建 `~/.ctrl/hermes-venv` → `pip install hermes-agent` → `hermes --version` 验证 → `hermes gateway --port 8642` 起服务.

*(Bootstrap step list elided — landed in spec v0.2 §3.3.)*
...
**Required change**: §3.4 加 zeus / hephaestus verify 步骤 — 启动 `hermes serve`, 用 `hermes token` 拿 bearer, POST `/v1/runs` 提交 minimal message, 然后 `curl -N` 流式拉 `/runs/$RUN_ID/events` SSE.

*(Spike commands elided — full snippet landed in spec v0.2 §3.4.)*
...
大概率 MIT 同 org 风格. hephaestus 用前 verify via `gh repo view NousResearch/hermes-example-plugins --json licenseInfo`, 然后更新 §5.5 table.
