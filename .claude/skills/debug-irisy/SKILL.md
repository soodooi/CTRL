...
3. **agent 自己跑探针 — 不要外包给用户**(bao 2026-06-29: "你不能直接调试吗?直接看 Irisy 真实回复?")。直接起 Irisy 的引擎拿真实回复:
   ...
   ```
   这条**真实调模型**(同一引擎、同一 `~/.hermes/SOUL.md`), 是 app 内 ACP 路径的忠实代理 —— 魂层一致。差别: app 内还叠加 `acp_client.rs::CTRL_CAPABILITY_BRIEF`(oneshot 没有), 所以 brief 引起的问题要在 app 内复测。其它有用的自助命令: `hermes status` / `hermes doctor` / `hermes logs -f --component tools` / `hermes config`。
